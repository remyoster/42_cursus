...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/02/03 17:35:19 by lojesu            #+#    #+#             */
/*   Updated: 2019/02/11 19:18:00 by lojesu           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_bearing_cold(t_win *win, int max, int min, int i)
{
	if (i < (max - min) / 6)
	{
		win->rgb.r = 102 - i * (102 / ((max - min) / 6));
		win->rgb.g = 0;
		win->rgb.b = 204 + i * (51 / ((max - min) / 6));
	}
	if (i >= (max - min) / 6 && i < (2 * ((max - min) / 6)))
	{
		win->rgb.r = 0;
		win->rgb.g = 0 + (i - ((max - min) / 6)) * (255 / ((max - min) / 6));
		win->rgb.b = 255 - (i - ((max - min) / 6)) * (255 / ((max - min) / 6));
	}
	if (i >= (2 * ((max - min) / 6)) && i < (3 * ((max - min) / 6)))
	{
		win->rgb.r = 0 + (i - 2 * ((max - min) / 6)) *
			(255 / ((max - min) / 6));
		win->rgb.g = 255;
		win->rgb.b = 0;
	}
...
static void	ft_special_rainbow(t_win *win, int max, int min, int *tab)
{
	int		i;
...
void		ft_rainbow(t_win win)
{
	...
	if (!(tab = (int*)malloc(sizeof(int) * (max - min) + 2)))
		return ;
	if ((max - min) < 6)
		ft_special_rainbow(&win, max, min, tab);
	else
	{
		while (i <= max - min)
		{
