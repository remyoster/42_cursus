...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_fill_window_para.c                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: roster <marvin@42.fr>                      +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/02/07 14:52:17 by roster            #+#    #+#             */
/*   Updated: 2019/02/12 13:30:25 by roster           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_do_the_math2(t_win *win, t_crd *crd, size_t i, size_t j)
{
	...
	{
		crd->x0 = win->evt.dx;
		crd->y0 = win->evt.dy - win->cst_proj * win->pix[i][j].z
			* win->evt.gap_y / 5;
		if (win->pix[i][j].h > -1)
		...
		crd->x1 = win->evt.dx;
		crd->y1 = win->evt.dy - win->cst_proj * win->pix[i + 1][j].z
			* win->evt.gap_y / 5;
...
static void	ft_fill_window_para_part2(t_win *win)
{
	size_t	i;
	size_t	j;
	t_crd	crd;
...
static void	ft_do_the_math(t_win *win, t_crd *crd, size_t i, size_t j)
{
	while (j < win->pos.x - 1)
	{
		...
		win->evt.dy = win->evt.dy + win->evt.gap_y;
		crd->x1 = win->evt.dx;
		crd->y1 = win->evt.dy - win->cst_proj * win->pix[i][j + 1].z
			* win->evt.gap_y / 5;
